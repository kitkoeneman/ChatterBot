...
from suggestion_tree import get_suggestion_text
from suggestion_tree import mark_discussed
from suggestion_tree import make_tree
from suggestion_tree import get_random_node_not_root
...
class ChatBot(object):
    ...
    def __init__(self, name, **kwargs):
        self.name = name
        self.last_statement_bot_sent = None

        self.suggestions = make_tree()
    ...
    def get_last_message_bot_sent(self, conversation):
        """
        Returns the latest response in a conversation if it exists.
        Returns None if a matching conversation cannot be found.
        """
        ...
        # Get the most recent statement in the conversation if one exists
        latest_statement = conversation_statements[-2] if conversation_statements else None
    ...
    def get_last(self):
        '''
        An alternative to get_last_message_bot_sent(self, conversation)
        Figuring out how to get the current conversation proved to be quite difficult so this function saved some time.

        :return: Statement object, belonging to the chatbot instance.
        '''
        return self.last_statement_bot_sent

    def set_last(self, statement):
        self.last_statement_bot_sent = statement

    def get_suggestion(self):
        '''
        Allows the chatbot to get textual representations of suggestions that it can send to the user.

        :return: A textual message the chatbot sends to the user to try and spark a conversation thread.
        '''
        return get_suggestion_text(self.suggestions)

    def get_random_suggestion(self):
        '''
        Used once all nodes in the tree are True. It will revisit random nodes to give suggestions.
        Some lower down nodes may have been overlooked due to the fact that suggestions are pulled from the top of the tree.

        :return: A textual message the chatbot sends to the user to try and spark a conversation thread.
        '''
        return get_suggestion_text(get_random_node_not_root(self.suggestions))

    def check_if_suggestion(self, statement):
        '''
        Checks if the Statement sent by the chatbot corresponds to a node in the suggestion tree. If it is the appropriate
        nodes in the suggestion tree will be marked discussed.

        :param statement: Statement object
        :return: No return
        '''
        try:
            # statement from chatbot.py is text only.
            # Get generator object using storage.filter, use next to get full Statement object w/ tags
            stmnt = next(self.storage.filter(text=statement.text))
            tags = stmnt.get_tags()
            for tag in tags:
                if tag[:11] == 'SUGGESTION:':
                    mark_discussed(self.suggestions, tag[11:])
